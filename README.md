...
operating on it, not the platform itself. The platform/home domain is
`cmbreathe.com`; `pennpaps.com` routes only to the Penn tenant. PennPaps's
own branding, contact addresses, and storefront copy are tenant data; additional
...
`Dockerfile` is required. Custom domain → Railway → DNS; the platform/home
domain is `cmbreathe.com` and the fallback host is `pennfit.up.railway.app`.
Environment variables are configured under **Variables** in the Railway service settings; the required set is
...
- pnpm **v11+** (pinned to `pnpm@11.7.0` via `packageManager`).
...
| `SHOP_PUBLIC_BASE_URL`, `REMINDER_PUBLIC_BASE_URL`, `RESUPPLY_VOICE_PUBLIC_BASE_URL`                                                                                                                                                                                                                                                                                                                                        | `resupply-api`        | Public base URLs used in outbound deep links. Cart-recovery + reminder emails fall through `SHOP → RESUPPLY_VOICE → https://pennpaps.com` in order for the Penn tenant; platform links should use `https://cmbreathe.com`.                                                                                                                                                                                                                                                                                                                 |
| `SENDGRID_API_KEY`, `SENDGRID_FROM_EMAIL`, `SENDGRID_FROM_NAME`, `SENDGRID_EVENT_WEBHOOK_PUBLIC_KEY`                                                                                                                                                                                                                                                                                                                        | `resupply-api`        | Outbound email + delivery webhooks (used by both the request-handling routes and the in-process worker's reminder jobs). Every sender across the monorepo funnels through the shared `createSendgridClient()` in `lib/resupply-email`, so `SENDGRID_FROM_EMAIL` (platform default `noreply@cmbreathe.com`) is the default From for the platform; a per-tenant `organizations.from_email` overrides it (the Penn Home Medical Supply tenant is pinned to `info@pennpaps.com` via migration 0377). Email features log-and-skip when missing. |
...
| `RESUPPLY_TURN_URLS`, `RESUPPLY_TURN_USERNAME`, `RESUPPLY_TURN_CREDENTIAL`                                                                                                                                                                                                                                                                                                                                                  | `resupply-api`        | Optional static TURN relay for telehealth video visits. When unset and Twilio credentials are present, ephemeral relay credentials are minted automatically via Twilio Network Traversal Service; when neither is available, calls fall back to public STUN only.                                                                                                                                                                                                                                                                          |
| `OPENAI_API_KEY`                                                                                                                                                                                                                                                                                                                                                                                                            | `resupply-api`        | Conversation AI. AI features disable when missing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `ANTHROPIC_API_KEY`, `DEEPGRAM_API_KEY`, `ELEVENLABS_API_KEY` (+ `ELEVENLABS_VOICE_ID` / `_MODEL_ID` / `_STABILITY` / `_SPEED` / `_TTS_TRANSPORT`)                                                                                                                                                                                                                                                                          | `resupply-api`        | The other two AI vendors plus ElevenLabs TTS tuning (see CLAUDE.md "AI / communications stack"). When `ANTHROPIC_API_KEY` is set, Claude takes priority for every text-only LLM call (chatbots, sleep coach, SMS classifier, post-call summaries) with OpenAI as fallback. Deepgram adds an audit-grade backup transcript on voice calls. ElevenLabs becomes the voice agent's TTS voice; unset → OpenAI's built-in `cedar`. All degrade gracefully when missing.                                                                          |
| `TELNYX_API_KEY`, `TELNYX_FAX_CONNECTION_ID`, `TELNYX_FAX_FROM_NUMBER`, `TELNYX_PUBLIC_KEY`                                                                                                                                                                                                                                                                                                                                 | `resupply-api`        | Telnyx Programmable Fax (outbound Rx-request faxes + inbound fax webhooks). Fax sending is disabled when the first three are unset; without `TELNYX_PUBLIC_KEY` the fax webhook routes reject every request (signature can't be verified).                                                                                                                                                                                                                                                                                                 |
| `WEB_PUSH_VAPID_PUBLIC_KEY`, `WEB_PUSH_VAPID_PRIVATE_KEY`, `WEB_PUSH_VAPID_SUBJECT`                                                                                                                                                                                                                                                                                                                                         | `resupply-api`        | Web Push (VAPID) browser notifications. When unset the public-key endpoint returns 503 and the SPA hides its "Enable push" toggle.                                                                                                                                                                                                                                                                                                                                                                                                         |
| `STRIPE_SECRET_KEY`                                                                                                                                                                                                                                                                                                                                                                                                         | `resupply-api`        | Cash-pay shop checkout + webhooks. Shop endpoints return preview-mode responses when missing.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `STRIPE_PLATFORM_SECRET_KEY`, `STRIPE_PLATFORM_WEBHOOK_SIGNING_SECRET`                                                                                                                                                                                                                                                                                                                                                      | `resupply-api`        | OPTIONAL dedicated Stripe account for PLATFORM SaaS billing (tenants paying the platform), kept off the patient-checkout account. Unset → platform billing shares `STRIPE_SECRET_KEY` (single-account mode). When set, the dedicated account's webhook posts to `/resupply-api/stripe/platform-webhook` (verified with the platform signing secret); both must be set together (enforced by `preflight:prod`).                                                                                                                             |
...
| `RESUPPLY_FITTER_FIRST_DAY_NUDGE_ENABLED`, `RESUPPLY_FITTER_SUPPLY_CAMPAIGN_ENABLED`, `RESUPPLY_REFIT_CAMPAIGN_ENABLED`, `RESUPPLY_PRESCRIPTION_AUTO_DRAFT_ENABLED`, `RESUPPLY_FITTER_REENGAGE_ENABLED`, `RESUPPLY_CART_ABANDONMENT_CRON_ENABLED`, `RESUPPLY_COACHING_AUTO_ENROLL_ENABLED`, `RESUPPLY_FAILED_EMAIL_DIGEST_ENABLED`, `RESUPPLY_DEMO_DRIP_ENABLED`, `FITTER_SUPPLY_CAMPAIGN_PROMO`, `FITTER_FINAL_CALL_PROMO` | `resupply-api` worker | Env-gated pg-boss dispatchers (`1` to enable; all OFF by default so a credentialed staging deploy can't auto-message real patients or carts) + optional promo codes surfaced in the fitter supply-campaign emails. Most other toggles now live in the DB `feature_flags` table (admin Control Center), which also provides a runtime kill switch per dispatcher.                                                                                                                                                                           |
...
| `ELIGIBILITY_REVERIFY_CRON`, `CLAIMS_AUTOSUBMIT_CRON`, `BILL_HOLD_SWEEP_CRON`, `CLINICAL_OUTREACH_CRON`, `RESUPPLY_SLA_ESCALATION_CRON`, `BILLING_PATIENT_AUTOPAY_CRON`, `BILLING_PAYMENT_PLAN_AUTOCHARGE_CRON`, `PATIENT_PACKET_REMINDER_CRON`                                                                                                                                                                             | `resupply-api` worker | Opt-in 5-field cron expressions that attach recurring schedules to pg-boss batch jobs (eligibility 270s, claim auto-submit, bill-hold sweep, clinical outreach, SLA escalation, patient autopay, payment-plan autocharge, packet reminders). Unset → no recurring schedule; the operator "Run now" triggers always work, and the money-moving jobs additionally require their feature flag + per-patient Stripe authorization. `PATIENT_PACKET_REMINDER_CRON` is the exception with a default (`33 19 * * *`).                             |
| `RESUPPLY_PRACTICE_NAME`, `RESUPPLY_PRACTICE_PHONE`, `RESUPPLY_PRACTICE_TIMEZONE`, `RESUPPLY_COACH_PHONE`                                                                                                                                                                                                                                                                                                                   | `resupply-api`        | Practice identity rendered in patient-facing copy: display name in reminder/voice/messaging copy (default `PennPaps`; once the admin saves the Company information page the database name wins — the API re-hydrates this variable at boot and on save); phone printed on prior-auth docs (omitted when blank); IANA timezone for appointment-email date formatting (default `America/New_York`); coach phone in low-usage check-in alerts (that alert is skipped when blank).                                                             |
| `RESUPPLY_ADMIN_ALERTS_EMAIL`, `SHOP_CSR_INBOX_EMAIL`                                                                                                                                                                                                                                                                                                                                                                       | `resupply-api`        | Ops alert recipients: the failed-order-email digest, and "customer messaged you" pings from signed-in shoppers. Blank → inherit `OPS_EMAIL`; when still empty the alert is logged and skipped (no error).                                                                                                                                                                                                                                                                                                                                  |
| `RESUPPLY_CHAT_GLOBAL_TURNS_PER_MINUTE`, `RESUPPLY_EMAIL_AUTO_REPLY_MIN_CONFIDENCE`, `PGBOSS_POOL_MAX`, `AUTH_SESSION_TTL_DAYS`, `AUTH_EMAIL_TOKEN_TTL_HOURS`, `PENN_ADMIN_PUBLIC_BASE_URL`, `OPENAI_REALTIME_DIAGNOSTIC_ENABLED`, `PACWARE_EXCHANGE_DISABLED`, `APP_CONFIG_OVERLAY_DISABLED`, `ADHERENCE_MODEL_JSON`                                                                                                       | `resupply-api`        | More tuning, each with a sensible default: global `/api/chat` turn budget (120/min); inbound-email auto-reply confidence bar (0.8); pg-boss pool size (5); session / email-token lifetimes (14 d / 24 h); admin-console origin used in invite emails; voice diagnostic test line (off); PacWare exchange-surface kill switch; `app_config` overlay kill switch; optional trained adherence-model JSON (built-in heuristic when unset).                                                                                                     |
| `RUN_DB_MIGRATIONS`, `MIGRATIONS_BASELINE_THROUGH`, `MIGRATIONS_BASELINE_EXCEPT`                                                                                                                                                                                                                                                                                                                                            | deploy (Railway)      | Read by `lib/resupply-db/scripts/deploy-migrate.mjs` (the `railway.json` `preDeployCommand`). Migrations run only when `RUN_DB_MIGRATIONS` is exactly `true`; the `MIGRATIONS_BASELINE_*` pair is only for one-time ledger adoption of an existing DB (see [`docs/runbooks/adopt-migration-ledger.md`](./docs/runbooks/adopt-migration-ledger.md)).                                                                                                                                                                                        |
