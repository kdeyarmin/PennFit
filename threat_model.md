...
**Host-header → tenant spoofing (known, edge-mitigated).** Multi-tenant resolution keys off the request host (`requestHost()` in `artifacts/resupply-api/src/lib/request-host.ts` → `resolveOrgIdByHost`), which prefers Express's trust-proxy-aware `req.hostname`. Express computes `req.hostname` from `X-Forwarded-Host` whenever the immediate connection peer is trusted, and `createTrustProxyFn()` (`lib/trusted-proxies.ts`) trusts hop 0 unconditionally because on Railway the socket peer is always the platform's internal load balancer (never the client). Consequently a forged `X-Forwarded-Host` is only neutralized if the **edge strips client-supplied `X-Forwarded-Host` before it reaches Express** — otherwise a request to one tenant's domain could set `X-Forwarded-Host` to another _registered_ custom domain and have leads/orders filed under, or outbound mail branded as, that victim tenant. Note the bound: the spoofed host only resolves to a tenant if it matches a registered custom domain, and host selects which tenant a request **writes/brands as**, not which tenant's data it can **read** — so the impact is write-misattribution / branding-spoof, not cross-tenant disclosure. The mitigation is edge configuration (Cloudflare Transform Rule + Railway) that removes any inbound `X-Forwarded-Host` so only the trusted edge sets it; see [`docs/runbooks/edge-header-hardening.md`](docs/runbooks/edge-header-hardening.md). Do **not** "fix" this by validating hop 0 against proxy CIDRs in `createTrustProxyFn()` — the immediate peer is Railway's internal LB (not a Cloudflare CIDR), so that would also break `req.ip` real-client resolution and custom-domain routing.
