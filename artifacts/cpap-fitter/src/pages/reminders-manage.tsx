...
function buildState(
  serverItems: Array<{ sku: string; lastReplacedAt: string; intervalDays: number }>,
...
export function RemindersManage() {
      ...
      <>
        ...
        <Card className="border-0 glass-card rounded-2xl">
          <CardHeader className="text-center space-y-3">
            ...
            <CardTitle>
              {status === 404 ? "Subscription not found" : "Could not load subscription"}
            </CardTitle>
            <CardDescription>
              {status === 404
  ...
  function onSave() {
    if (!items) return;
    const enabled = REMINDER_ITEMS.filter((d) => items[d.sku].enabled).map((d) => ({
      sku: d.sku,
  ...
  function onUnsubscribe() {
    unsub.mutate({ params: { token } }, { onSuccess: () => setUnsubscribed(true) });
  }
    ...
    <>
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Reminders for <span className="font-medium">{data.email}</span>. Update
          your dates after you swap supplies so we don't ping you about
      ...
      <Card className="border-0 glass-card rounded-2xl">
        ...
        <CardContent>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <Label htmlFor={`m-${def.sku}`} className="text-base cursor-pointer">
                        {def.label}
                      </Label>
                  ...
                  {state.enabled && (
                    <div className="grid sm:grid-cols-2 gap-3 pl-7">
                      <div className="space-y-1">
                        <Label htmlFor={`m-last-${def.sku}`} className="text-xs text-muted-foreground">
                          Last replaced
                        </Label>
                        <Input
                          id={`m-last-${def.sku}`}
                          ...
                          onChange={(e) =>
                            updateItemField(def.sku, { lastReplacedAt: e.target.value })
                          }
                          data-testid={`input-manage-last-${def.sku}`}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label htmlFor={`m-int-${def.sku}`} className="text-xs text-muted-foreground">
                          Remind every (days)
                        </Label>
                        <Input
                          id={`m-int-${def.sku}`}
          ...
          <div className="flex flex-wrap items-center gap-3 mt-6">
            ...
            >
              {unsub.isPending ? "Unsubscribing..." : "Unsubscribe from all reminders"}
            </Button>
          </div>
        </CardContent>
      </Card>
