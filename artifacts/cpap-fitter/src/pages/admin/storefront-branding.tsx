...
export function AdminStorefrontBrandingPage() {
  ...
  const domainVerify = useMutation({
    ...
    onSuccess: (next) => {
      ...
      setVerifyMsg(
        next.verified
          ? "Domain verified — your storefront is now live on this domain."
  ...
  return (
    <div className="admin-root p-6 space-y-6 max-w-4xl">
      ...
      <Card title="Custom domain">
        ...
        {/* TLS / HTTPS for custom domains is handled at the infrastructure
            level (Railway + custom domain provider). */}
      </Card>
    </div>
  );
}
