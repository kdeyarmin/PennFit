// Public "review & sign" page for CSR-created orders.
//
// The customer arrives via a signed HMAC link (/order-sign?token=...)
// sent by a CSR from the admin Orders page. Two steps on one page:
//   1. Review the order (line items + note from the team).
//   2. Read + acknowledge each paperwork document, then e-sign
//      (typed or drawn — same UX as the patient-packet signing page).
//
// Nothing is charged: once signed, the order is billed to the patient's
...
const PAGE_TITLE = "Review & sign for your order";
...
export function OrderSign() {
  ...
  // Read the token once. It stays available in state for the sign call;
  // we strip it from the URL so the personalized link doesn't linger in
  // history.
    ...
    <PageShell>
      ...
      <div className="mb-6 space-y-3">
        ...
        <p className="text-slate-600">
          ...
          {documents.length > 0
            ? "please review your order and sign the paperwork below."
            : "please review your order and add your signature."}
        </p>
        ...
        <Card className="border-emerald-200">
          <CardHeader>
            ...
            <CardDescription>
              ...
              We'll bill your insurance and take it from here.
            </CardDescription>
          </CardHeader>
          <CardContent className="text-center">
            <Button asChild>
...
function friendlyError(err: unknown): string {
  ...
  if (/already_signed/.test(raw)) return "This order has already been signed.";
  if (/signature_required/.test(raw))
    return "Please sign the paperwork above to continue.";
  if (/expired|canceled|invalid|not_found/.test(raw))
    return "This order link is no longer valid. Please contact us for a new one.";
  return raw || "We couldn't submit your signature. Please try again.";
}
