  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        ...
        {!oneTimeOutOfStock && (
          <ShippingEta className="mt-4 max-w-sm" />
        )}
        <Link
          href="/insurance"
...
function easternPartsNow(now: Date): EasternParts {
  // Intl gives us wall-clock parts in the target tz without pulling
  // in a date library. Locale is en-US so we get predictable tokens.
  ...
  // The hour token comes back as "24" at midnight in some Node
  // versions; normalize to 0.
...
// Add `n` calendar days to a y/m/d triple via Date arithmetic in UTC,
// which is exact for whole-day offsets (no DST drift).
...
function nextBusinessDay(year: number, month: number, day: number) {
  ...
  while (!isBusinessDay(cur.weekday)) cur = addDays(cur.year, cur.month, cur.day, 1);
  return cur;
}
...
function addBusinessDays(
  ...
  let cur = { year, month, day, weekday: new Date(Date.UTC(year, month - 1, day)).getUTCDay() };
  let added = 0;
  while (added < n) {
...
function formatDayLabel(year: number, month: number, day: number): string {
  const d = new Date(Date.UTC(year, month - 1, day, 12)); // noon UTC to avoid tz wobble
...
function computeShippingEta(now: Date): ShippingEtaState {
  const et = easternPartsNow(now);
  const isWeekend = !isBusinessDay(et.weekday);
  const beforeCutoff =
    !isWeekend &&
    (et.hour < SHIP_CUTOFF_HOUR_ET ||
      (et.hour === SHIP_CUTOFF_HOUR_ET && et.minute === 0 && false));
  // (the false-AND keeps the cutoff strict: 14:00:00 ET is already past)
  ...
  const arrivalLine = `Estimated delivery ${formatDayLabel(earliest.year, earliest.month, earliest.day)} – ${formatDayLabel(latest.year, latest.month, latest.day)}`;
  return { ctaLine, arrivalLine, countdown };
}

function ShippingEta({ className = "" }: { className?: string }) {
  // Tick once a minute so the countdown stays fresh without churning
  // CPU. We also re-compute on mount so SSR/hydration mismatches
  // don't leave a stale value visible.
  ...
  return (
    ...
    >
      ...
      <div className="leading-snug">
        <div
          className="font-semibold"
          data-testid="pdp-shipping-eta-cta"
        ...
        <div
          ...
          data-testid="pdp-shipping-eta-arrival"
        >
          {arrivalLine}
        </div>
      </div>
