...
describe("reminders-manage — regression: core manage behaviour intact", () => {
  ...
  it("calls update.mutate with the token (when present) and enabled items on Save", () => {
    ...
    // The page supports both magic-link tokens and session auth; the
    // params object only carries the token when one is present.
    expect(SRC).toContain("params: hasToken ? { token } : {}");
  });
