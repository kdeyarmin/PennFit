...
// ---------------------------------------------------------------------------
// RemindersSection — present in account.tsx (regression guard)
// ---------------------------------------------------------------------------
// The RemindersSection tile (linking /account → /reminders/manage) is active
// in the current source. These tests guard against the tile being accidentally
// removed in a future refactor.

describe("account — RemindersSection present and rendered", () => {
  it("defines RemindersSection as a local function", () => {
    expect(SRC).toContain("function RemindersSection");
  });

  it("renders <RemindersSection /> in the JSX tree", () => {
  ...
  it("has data-testid='account-reminders-section' on the section wrapper", () => {
    expect(SRC).toContain('data-testid="account-reminders-section"');
  });

  it("links to /reminders/manage with data-testid='account-link-reminders-manage'", () => {
    expect(SRC).toContain('data-testid="account-link-reminders-manage"');
    expect(SRC).toContain('href="/reminders/manage"');
  });

  it("includes 'Skip the inbox round-trip' value-proposition copy", () => {
    expect(SRC).toContain("Skip the inbox round-trip");
  });

  it("also provides a link to /reminders for customers who haven't subscribed yet", () => {
