...
const EMPTY_COLLECTION_KEYS = [
  ...
  // ── Drift sweep ────────────────────────────────────────────────
  // Every remaining array-typed field declared on an API response type
  // in the SPA's client layer. Generated by the companion drift test
  // (empty.drift.test.ts), which fails if a NEW array-typed response
  // field is added without being seeded here — the "a missing name is a
  // latent crash" rule in this file's header, enforced instead of
  // hoped for. Keep alphabetical so additions diff cleanly.
