...
// populated yet, so the key is in practice ALWAYS the IP. The admin
// shell alone polls two endpoints every 60s per open tab (~120 reads/hr)
// before anyone clicks anything, so the original 600/hr cap starved
// honest consoles: once the bucket drained, every admin GET 429'd and
// surfaced as random broken widgets (e.g. the Documents page type dropdown
// rendering empty). 6000/hr (~1.7 req/s sustained) still bounds
// scraping/abuse while absorbing a busy multi-tab, multi-staff office
// on one IP.
...
// so the key is in practice ALWAYS the IP. 1500/hr is well above any
// honest office's combined CSR mutation cadence while still bounding a
