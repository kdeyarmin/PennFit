...
//
// The app runs behind Railway's reverse proxy, so the browser-visible
// host arrives in `X-Forwarded-Host`, not `Host`. Tenant resolution
// (host → org_id / branding) keys off this value, so every caller must
// read it the same way — hence one shared helper instead of per-route
// copies that were drifting apart.
