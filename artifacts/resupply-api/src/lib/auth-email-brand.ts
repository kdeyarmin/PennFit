 ...
 *
 * Link origin follows the same rule: only a verified custom domain (via
 * `resolveBrandOrgIdByHost` → `resolveTenantBaseUrl`) may override the
 * mount's platform `publicBaseUrl`. Raw Host is never trusted for links.
 *
...
export const storefrontAuthBrandResolver: AuthBrandResolver = async (req) => {
  ...
  return {
    ...
    ...(tenantBase ? { publicBaseUrl: tenantBase.replace(/\/$/, "") } : {}),
  };
};
