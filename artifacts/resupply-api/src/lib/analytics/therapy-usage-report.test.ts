...
describe("aggregateTherapyUsageReport", () => {
  it("computes per-bucket usage, adherence, and CMS compliance (real rule)", () => {
    // p1: 30 consecutive 5h nights → a qualifying CMS window → compliant.
    // p2: 5 consecutive 5h nights → no 30-day window can qualify (only 5
    //     of 30 calendar days compliant = 17% < 70%) → not compliant.
    const nights = [
      ...run("p1", "2026-01-01", 30, 300),
      ...run("p2", "2026-02-01", 5, 300),
    ];
  ...
  it("treats null usage minutes as a non-adherent zero night", () => {
    const nights = [
      { patientId: "p1", date: "2026-05-01", usageMinutes: null, ahi: null, leakRateLMin: null },
      { patientId: "p1", date: "2026-05-02", usageMinutes: 300, ahi: null, leakRateLMin: null },
    ];
    const r = aggregateTherapyUsageReport({
  ...
  it("dedups a patient who appears under multiple buckets in the summary", () => {
    ...
    const nights = run("p1", "2026-01-01", 30, 300);
    const buckets = new Map<string, GroupRef[]>([
      ["p1", [
  ...
  it("averages AHI and leak only over nights that reported them", () => {
    const nights: PatientNight[] = [
      { patientId: "p1", date: "2026-05-01", usageMinutes: 300, ahi: 2, leakRateLMin: 10 },
      { patientId: "p1", date: "2026-05-02", usageMinutes: 300, ahi: 4, leakRateLMin: null },
      { patientId: "p1", date: "2026-05-03", usageMinutes: 300, ahi: null, leakRateLMin: null },
    ];
    const r = aggregateTherapyUsageReport({
      grouping: "manufacturer",
      nights,
