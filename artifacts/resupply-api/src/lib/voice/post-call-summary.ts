...
const SYSTEM_PROMPT = [
  ...
  "- `sentiment` is the PATIENT's overall tone — judge it from what the",
  "  PATIENT says and how they say it, NOT from the agent's politeness:",
  ...
  "  When the tone shifts during the call, report where it ENDED — a",
  "  caller who started frustrated but left satisfied is positive.",
  ...
  "  Use plain English. Put the most safety-relevant item FIRST. Empty",
  "  array if the call was a routine refill with nothing raised.",
  ...
  '  email". Be specific about WHAT was promised so ops can verify it',
  "  actually happened. Empty array if the agent made no commitments",
  ...
  "  legitimate, any safety signal, OR any agent commitment in",
  "  `followUps` that needs a human to carry out. Whenever you set this",
  "  TRUE, make sure the REASON is captured in `concerns` or `followUps`",
  "  so the reviewer can see why without re-reading the whole call.",
