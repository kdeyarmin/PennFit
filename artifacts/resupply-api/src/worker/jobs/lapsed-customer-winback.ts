...
export async function runLapsedCustomerWinback(): Promise<WinbackStats> {
  ...
  // 1. Pull a candidate slate. We over-fetch by a margin because
  //    the per-row gates (active recent order existence, lapsed
  //    threshold, comm-prefs) will filter further. Keeping this
  //    SELECT lean — no JSON, no order joins — keeps the DB hit
  //    cheap; we do the per-customer lookups inside the loop where
  //    we can short-circuit the cohort fan-in.
