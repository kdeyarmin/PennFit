...
// /resupply-api/me — operator identity smoke endpoint.
//
// Why this exists:
//   The dashboard needs a single, cheap call after sign-in to ask
//   "am I authorized as an operator on THIS server, and what email
//   does the API see for me?". That answer drives:
//     - Whether to show the operator UI at all (200 = show, 403 =
...
//   Clerk user object, or the operator allowlist — only the two
...
//
// Auth:
//   `requireOperator` runs first. By the time the handler executes,
//   it has already proven:
//     1. There is a valid Clerk session (else 401),
//     2. The session's primary email is verified (else 403),
//     3. The email is on the operator allowlist (else 403),
//   AND attached `operatorEmail` + `operatorClerkId` to `req`.
//   The handler itself never reaches Clerk and never re-validates.

const router: IRouter = Router();

router.get("/me", requireOperator, (req, res) => {
  // Both fields are guaranteed to be set by requireOperator on the
