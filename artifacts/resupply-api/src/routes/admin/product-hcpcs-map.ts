...
const LOOKUP_KIND_VALUES = [
  "stripe_product_id",
  "item_sku",
] as const satisfies readonly Row["lookup_kind"][];
...
router.get(
  ...
  async (req, res) => {
    ...
    let query = supabase
      ...
      .limit(500);
    const lookupKind =
      typeof req.query.lookupKind === "string"
        ? req.query.lookupKind
        : undefined;
    if (
      lookupKind &&
      (LOOKUP_KIND_VALUES as readonly string[]).includes(lookupKind)
    ) {
...
router.post(
  ...
  async (req, res) => {
    ...
    await logAudit({
      ...
      logger.warn(
        { err: redactDbErr(err) },
        "product_hcpcs_map.create audit write failed",
      );
    });
...
router.patch(
  ...
  async (req, res) => {
    ...
    if (b.defaultModifiers !== undefined)
      update.default_modifiers = b.defaultModifiers;
    if (b.unitsPerDispense !== undefined)
      update.units_per_dispense = b.unitsPerDispense;
    if (b.defaultBilledCents !== undefined)
      update.default_billed_cents = b.defaultBilledCents;
    ...
    await logAudit({
      ...
      logger.warn(
        { err: redactDbErr(err) },
        "product_hcpcs_map.update audit write failed",
      );
    });
    res.json({ ok: true });
  },
