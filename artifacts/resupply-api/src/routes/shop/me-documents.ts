  ...
  async (req, res) => {
    ...
    if (!ALLOWED_DOCUMENT_TYPES.has(body.data.documentType)) {
      res.status(400).json({
        error: "invalid_body",
        issues: [
          { path: "documentType", message: "unsupported document type" },
        ],
      });
      return;
    }
    ...
    try {
      ...
      await logAudit({
        ...
        logger.warn(
          { err },
          "patient.document.upload_url_issued audit write failed",
        );
      });
...
router.post("/shop/me/documents", requireSignedIn, async (req, res) => {
  const customerEmail = req.shopCustomerEmail;
  if (!customerEmail) {
    res.status(404).json({ error: "patient_not_found" });
    return;
...
router.get("/shop/me/documents", requireSignedIn, async (req, res) => {
  const customerEmail = req.shopCustomerEmail;
  if (!customerEmail) {
    res.json({ documents: [] });
    return;
...
router.get("/shop/me/documents/:docId", requireSignedIn, async (req, res) => {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (err instanceof ObjectNotFoundError) {
        ...
        req.log.warn(
          { err, doc_id: doc.id },
          "patient_document_delete_bytes_failed",
        );
