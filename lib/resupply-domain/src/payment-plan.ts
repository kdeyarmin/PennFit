// Patient payment-plan math (biller #B7) — pure value-object logic
// (ADR 008: depends only on plain TypeScript, no I/O).
//
// Three responsibilities, all deterministic and unit-testable:
//   1. generateInstallmentSchedule — split a balance into N installments
//      on a weekly/biweekly/monthly cadence. The integer-division
//      remainder is added to the FIRST installment so the schedule always
//      sums EXACTLY to the total (no lost or phantom cents).
//   2. computePlanSummary — an A/R rollup (paid / remaining / overdue /
//      next-due) over a plan's installments as of a given day.
//   3. derivePlanStatus — the plan's lifecycle status (active vs
//      completed) from its installments.
//
// Money movement (Stripe auto-charge) is intentionally NOT here — this
// slice is the schedule + tracking the biller/CSR uses to manage a
// patient balance paid over time; charging is a follow-up. Relocated out
// of artifacts/resupply-api so the SPA can reuse the same tested math.
