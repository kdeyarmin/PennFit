 ...
 */
export const PROMPT_VERSION = "2026-09-24.v29" as const;
  ...
  if (callerKind === "breathe_prospect") {
    return [
      ...
      howToSpeak.replace(
        ...
        "If a tool result includes a URL, describe its purpose. Offer approved resource links through send_info_sms only with the caller's confirmed mobile number and explicit SMS permission. Never promise an automatic text after hangup. Describe submitted versus delivered exactly as the tool reports. Only confirm an email after an authorized email tool succeeds.",
      ),
