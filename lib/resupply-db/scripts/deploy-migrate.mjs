...
// RUN_DB_MIGRATIONS gate (see run-db-migrations-gate.mjs for the
// classification rules and the P2-16 background):
//   - truthy spellings (true/1/yes/on, any case)  → run migrations
//   - falsy spellings (unset/""/false/0/no/off)   → skip, exit 0
//   - anything else → FAIL the deploy. An operator who set the flag
//     intended migrations to run; silently skipping on a typo is the
//     dangerous outcome, so the typo must be loud.
const gate = classifyRunDbMigrations(process.env.RUN_DB_MIGRATIONS);
if (gate === "invalid") {
  process.stderr.write(
    `[deploy-migrate] RUN_DB_MIGRATIONS is set to ${JSON.stringify(
      process.env.RUN_DB_MIGRATIONS,
    )}, which is neither a recognized truthy value (true/1/yes/on) nor ` +
      "falsy value (false/0/no/off). Refusing to guess — failing the " +
      "deploy so a typo can't silently skip migrations.\n",
  );
  process.exit(1);
}
if (gate === "skip") {
...
// Deployment/database cross-check, evaluated BEFORE the migrator is
// spawned so the deploy log leads with the verdict and a refused deploy
// never opens a connection to the database it was told not to touch.
// migrate.mjs re-evaluates the same guard, so a manual invocation is
// protected too — this is the fast, legible copy, not the only one.
//
// Exit 3 (not 1) so "the guard stopped me" is distinguishable from "a
// migration failed" in Railway's deploy history. Either way the deploy
// is gated and the previous release keeps serving.
const guard = evaluateMigrationGuard(process.env);
process.stdout.write(formatGuardReport(guard));
if (!guard.allowed) {
  process.stderr.write(
    "[deploy-migrate] migration guard refused this deploy. No SQL was " +
      "executed. See docs/runbooks/migration-environment-guard.md.\n",
  );
  process.exit(3);
}
