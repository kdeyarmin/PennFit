...
async function main() {
  ...
  try {
    ...
    // One-time PHI-decryption assist for migration 0025_strip_phi_encryption.
    //
    // That migration converts the formerly pgcrypto-encrypted patient
    // PHI columns from `bytea` to plaintext `text` / `jsonb`. To
    // decrypt the existing ciphertext during the conversion it reads
    // the data key via `current_setting('app.data_key', true)`. We
    // expose the key here as a session-scoped GUC so:
    //   * It survives across drizzle's per-migration BEGIN/COMMIT
    //     (SET, not SET LOCAL).
    //   * It is never written to the SQL migration file (still in
    //     env only).
    //   * It is automatically scoped to this single connection /
    //     session and disappears when the connection closes.
    //
    // After 0025 has been applied this setting is harmless — no other
    // migration reads it. If RESUPPLY_DATA_KEY is unset we leave the
    // GUC unset entirely (current_setting(..., true) then returns
    // NULL); 0025 only fails if there is encrypted data AND no key,
    // which is exactly the case we want a loud failure for.
    const resupplyDataKey = process.env.RESUPPLY_DATA_KEY;
    if (resupplyDataKey && resupplyDataKey.trim() !== "") {
      await client.query("SELECT set_config('app.data_key', $1, false)", [
        resupplyDataKey,
      ]);
    }
