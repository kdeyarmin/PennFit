...
//
// Applies every pending migration in `lib/resupply-db/migrations/` to
// the database pointed to by DATABASE_URL, idempotently. Invoked by
// `scripts/post-merge.sh` as the deploy-time DB sync step.
//
// History: this script used to delegate the apply phase to a
// third-party ORM's `node-postgres` migrator. When that tooling was
...
// (`migrations.resupply_migrations(id SERIAL PK, hash text, created_at
// bigint)`) are preserved so a fresh checkout points at a
...
//
// One-time schema rename: the history schema was historically named
// `drizzle` (the retired ORM's default). It is now `migrations`. A
// populated production database still carries the rows under the old
// `drizzle` schema, so before reading the ledger we transparently
// `ALTER SCHEMA drizzle RENAME TO migrations` when the old schema
// exists and the new one does not. This preserves every applied-row
// hash (no replay) and is a no-op on already-renamed or fresh DBs.
//
...
//
// Migration history is stored in `migrations.resupply_migrations` so it
// never collides with the cpap-fitter's `public.*` tables. The
// `migrations` schema is created on demand below (and the legacy
// `drizzle` schema is renamed into it on first run against an older
// DB) so existing production rows continue to gate new migrations
// cleanly.
//
// Exit codes:
...
//   3 — the deployment/database guard refused the run (a non-production
//       deployment pointed at the production database, or an ambiguous
//       environment identity). No SQL executed. See
...
// Legacy schema name (the retired ORM's default). A populated DB
// provisioned before the rename still carries the ledger here; we
// rename it into MIGRATIONS_SCHEMA once, on first run.
...
// Supabase prohibits custom functions in its managed auth schema. Keep the
// historical files and ledger hashes intact; adapt only these reviewed bytes
// during replay. Migration 0546 converges databases that already ran them.
const AUTH_HELPER_COMPATIBILITY_HASHES = new Map([
  [
    "0059_auth_admin_enum_constraints",
...
const OWNED_AUTH_TIMESTAMP_HELPER = `
  CREATE OR REPLACE FUNCTION resupply_auth.set_updated_at()
  RETURNS trigger LANGUAGE plpgsql AS $$
  BEGIN NEW.updated_at = now(); RETURN NEW; END; $$;`;

export function migrationExecutionSql(migration) {
  const expectedHash = AUTH_HELPER_COMPATIBILITY_HASHES.get(migration.tag);
  if (!expectedHash) return migration.sql;
  const content = migration.sql.join("--> statement-breakpoint");
  if (
    migration.hash !== expectedHash ||
    crypto.createHash("sha256").update(content).digest("hex") !== expectedHash
  ) {
    throw new Error(
      `Historical auth-helper compatibility refused changed migration "${migration.tag}". ` +
        "Preserve the reviewed historical file and add a new migration instead.",
    );
  }
  const statements = migration.sql.map((statement) =>
    statement.replaceAll(
      /\bauth\.set_updated_at\(\)/g,
      "resupply_auth.set_updated_at()",
    ),
  );
  // 0059 may already be recorded with its legacy auth helper (or that helper
  // may have been removed by the platform), while 0060 still needs to run.
  if (migration.tag === "0060_updated_at_triggers_phase4") {
    statements.unshift(OWNED_AUTH_TIMESTAMP_HELPER);
  }
  return statements;
}
 ...
 * numeric prefix) and return the migration list. The legacy
 ...
 * `created_at` column stays byte-identical to the values the legacy
 * migrator wrote on past runs (so a production DB whose rows were
 * inserted by it still matches). New (non-journaled) files use the
...
export function readMigrations(folder) {
  ...
  // the original 0000..00NN journaled base PLUS one stray later entry
 ...
 *      Table shape is preserved exactly (id SERIAL PK + hash
 *      text + created_at bigint) so a database whose rows were
 *      written by the legacy migrator can be picked up
 *      without rewriting.
 ...
 *     diagnose and recover manually (matches the legacy migrator's
 *     previous behavior when an author dropped CONCURRENTLY to work
 *     around the wrap).
 *
...
export async function applyPendingMigrations(client, migrations) {
  ...
  for (const migration of migrations) {
    ...
    const executionSql = migrationExecutionSql(migration);
    if (executionSql !== migration.sql) {
      process.stdout.write(
        `[resupply-db-migrate] compatibility ${migration.tag}: ` +
          `auth.set_updated_at() -> resupply_auth.set_updated_at(); ` +
          `historical ledger hash unchanged (${migration.hash}).\n`,
      );
    }
    if (migration.noTransaction) {
      ...
      for (const stmt of executionSql) {
...
async function main() {
  ...
  // Deployment/database cross-check. A preview deployment that inherited
  // production's DATABASE_URL applied an unmerged migration to the
  // production database once; this is the gate that makes that
  // impossible. See lib/resupply-db/scripts/deploy-environment.mjs for
  // the identity rules and docs/runbooks/migration-environment-guard.md
  // for the variables to set. Exit code 3 is distinct from a migration
  // failure (1) and a missing DATABASE_URL (2) so a deploy log can be
  // triaged without reading it.
  const guard = evaluateMigrationGuard(process.env);
  process.stdout.write(formatGuardReport(guard));
  if (!guard.allowed) {
    process.stderr.write(
      "[resupply-db-migrate] migration guard refused this run. No SQL was executed.\n",
    );
    process.exit(3);
  }
  ...
  try {
    ...
    // One-time history-schema rename. A database provisioned before
    // the rename carries the ledger under the legacy `drizzle` schema.
    // Move it into MIGRATIONS_SCHEMA in place — metadata-only, cheap,
    // and it preserves every applied-row hash so the migrator still
    // considers the existing rows up-to-date (no replay). Only fires
    // when the legacy schema exists and the new one does not; a no-op
    // on already-renamed or brand-new databases.
    const schemaState = (
      await client.query(
        `SELECT to_regnamespace($1) IS NOT NULL AS legacy_exists,
                to_regnamespace($2) IS NOT NULL AS new_exists`,
        [LEGACY_MIGRATIONS_SCHEMA, MIGRATIONS_SCHEMA],
      )
    ).rows[0];
    if (schemaState?.legacy_exists && !schemaState?.new_exists) {
      await client.query(
        `ALTER SCHEMA "${LEGACY_MIGRATIONS_SCHEMA}" RENAME TO "${MIGRATIONS_SCHEMA}"`,
      );
      process.stdout.write(
        `[resupply-db-migrate] renamed legacy history schema ` +
          `"${LEGACY_MIGRATIONS_SCHEMA}" -> "${MIGRATIONS_SCHEMA}" ` +
          `(ledger preserved, no replay).\n`,
      );
    }
    ...
    // INSERT fails with `schema "migrations" does not exist`.
    ...
    // the legacy migrator path, before the schema-creation step was
      ...
      if (schemaPresent) {
        throw new Error(
          "Refusing to replay all migrations onto a populated database with no " +
            "migration ledger.\nThis database predates migrations.resupply_migrations. " +
