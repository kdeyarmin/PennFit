...
function readMigrations(folder) {
  ...
  for (const fileName of sqlFiles) {
    ...
    if (prior !== undefined) {
      throw new Error(
        `Duplicate migration prefix ${prefix}: both "${prior}" and "${tag}" share the same numeric prefix`,
      );
    }
    ...
    const noTransaction = /^[ \t]*--[ \t]*migrate:[ \t]*no-transaction/im.test(
      content.split("\n").slice(0, 5).join("\n"),
    );
  ...
  // Stable sort by `folderMillis` for two reasons:
  //   * journaled migrations keep their `when` timestamps so the
  //     history `created_at` column stays consistent with what
  //     Drizzle wrote;
  //   * unjournaled migrations sort by their numeric prefix, which
  //     interleaves correctly with any journaled migrations whose
  //     `when` is still less than the prefix (effectively, every
  //     post-2024 migration sorts strictly after every pre-2024
  //     journaled one).
  migrations.sort((a, b) => {
