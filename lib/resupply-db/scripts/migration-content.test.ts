...
describe("migration SQL content — static checks", () => {
  ...
  describe("0080_staff_training_records.sql", () => {
    ...
    it("includes the training_type CHECK constraint with all expected values", () => {
      ...
      for (const t of expectedTypes) {
        expect(
          sql,
          `training_type '${t}' must be in the CHECK constraint`,
        ).toContain(`'${t}'`);
      }
    });
    ...
    it("includes the expiry_after_completion CHECK constraint", () => {
      ...
      expect(sql).toMatch(
        /"expires_at" IS NULL OR "expires_at" >= "completed_at"/,
      );
    });
  ...
  describe("0087_accreditation_policies.sql", () => {
    ...
    it("does NOT declare staff_user_id as uuid in admin_policy_attestations", () => {
      ...
      const attestationsBlock = sql.slice(
        sql.indexOf('"resupply"."admin_policy_attestations"'),
      );
    ...
    it("references admin_users without ON DELETE CASCADE (soft FK for audit history)", () => {
      ...
      const staffFkLine = sql.match(
        /"staff_user_id" text NOT NULL REFERENCES[^\n]*/,
      )?.[0];
...
describe.skipIf(!dbUrl)("migration schema state — integration checks", () => {
  let pool: Pool;
  ...
  describe("0060 — trigger targets public.reminder_subscriptions", () => {
    it("trg_reminder_subscriptions_set_updated_at exists on public.reminder_subscriptions", async (ctx: TaskContext) => {
    ...
    it("trg_reminder_subscriptions_set_updated_at does NOT exist on resupply.reminder_subscriptions", async (ctx: TaskContext) => {
  ...
  describe("0061 — fulfillments.quantity is integer", () => {
    it("fulfillments.quantity has data_type 'integer'", async (ctx: TaskContext) => {
    ...
    it("fulfillments.quantity default is 1 (integer literal, not text '1')", async (ctx: TaskContext) => {
  ...
  for (const { schema, table } of staffUserIdTables) {
    describe(`${table} — staff_user_id column type`, () => {
      it(`${schema}.${table}.staff_user_id is data_type 'text' (not 'uuid')`, async (ctx: TaskContext) => {
  ...
  describe("0087 — admin_policy_attestations soft FK to admin_users", () => {
    it("FK from admin_policy_attestations.staff_user_id to admin_users has no CASCADE delete rule", async (ctx: TaskContext) => {
  ...
  describe("0090 — admin_users table exists", () => {
    it("resupply.admin_users table is present in the schema", async () => {
    ...
    it("resupply.admin_users.id is text (not uuid) with a non-null default", async (ctx: TaskContext) => {
      const res = await pool.query<{
        data_type: string;
        column_default: string | null;
      }>(
    ...
    it("resupply.admin_users.email_lower is text NOT NULL", async (ctx: TaskContext) => {
      const res = await pool.query<{
        data_type: string;
        is_nullable: string;
      }>(
