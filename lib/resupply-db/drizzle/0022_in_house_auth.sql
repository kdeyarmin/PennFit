...
--   * `auth.sessions.revoked_at` is the audit trail of "this token
--     was invalidated, here's when". A DELETE would lose that.
--   * `auth.login_attempts` is append-only by design (rate limiting
...
-- ---------------------------------------------------------------------------
-- auth.users — canonical identity row.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- auth.password_credentials — argon2id-hashed passwords.
-- ---------------------------------------------------------------------------
-- Split out from `auth.users` so a future read-only role can be
...
-- ---------------------------------------------------------------------------
-- auth.sessions — opaque server-issued session tokens.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- auth.email_tokens — single-use tokens delivered via email.
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS "auth_email_tokens_user_purpose_idx"
  ON "auth"."email_tokens" ("user_id", "purpose");

-- ---------------------------------------------------------------------------
-- auth.login_attempts — append-only sign-in attempt log.
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS "auth_login_attempts_email_idx"
  ON "auth"."login_attempts" ("email_lower", "attempted_at");
CREATE INDEX IF NOT EXISTS "auth_login_attempts_ip_idx"
  ON "auth"."login_attempts" ("ip", "attempted_at");
