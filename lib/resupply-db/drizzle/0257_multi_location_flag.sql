-- 0257_multi_location_flag — Control Center toggle for the multi-branch
-- (multi-location) feature.
--
-- (Re-adds the seed originally introduced as 0254; that number was taken
-- by migrations merged from main, so it is re-issued here at 0257. The
-- `multi_location.enabled` key is in FEATURE_FLAG_KEYS, so the catalog
-- drift test requires a seed — without this the flag would be unseeded
-- and isFeatureEnabled would default it ON in production.)
--
