...
// The `PennPaps` Supabase project (its project name, unrelated to the
// tenant's retired DBA) currently exposes only `public`, but
    ...
    Tables: {
      ...
      locations: {
        Row: {
          ...
          is_primary: boolean;
          is_active: boolean;
          // Per-branch billing identity (migration 0450). All nullable; the
          // anchor is `locations.npi` (there is no billing_npi column) — a
          // NULL/blank `npi` means "no location-level identity" and the
      ...
      shop_orders: {
        Row: {
          ...
          // Migration 0474: idempotency claim for the "your order
          ...
          // Migration 0475: idempotency/coordination claim for the
