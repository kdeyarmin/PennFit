...
| ID   | Item                                                                  | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| P0.1 | Drizzle journal drift                                                 | **OPEN — WORSE.** 52 journal entries vs 120 SQL files (68-file gap, up from 21 on 5/8). The new `.github/workflows/ci.yml:97` runs `scripts/check-drizzle-drift.sh` but with `continue-on-error: true`, so PRs don't fail. **NOT a code-only fix** — see [`migration-drift-status-2026-05-13.md`](./migration-drift-status-2026-05-13.md) and [`migration-state-investigation-2026-05-08.md`](./migration-state-investigation-2026-05-08.md): production-state inspection is gating. |
| P0.2 | Six duplicate migration prefixes (0016, 0017, 0049, 0050, 0052, 0065) | **OPEN — but secondary.** All six still present. The duplicates are mostly harmless (`drizzle-orm` matches by tag, not prefix); the 68-file journal gap (P0.1) is the load-bearing problem. See [`migration-drift-status-2026-05-13.md`](./migration-drift-status-2026-05-13.md).                                                                                                                                                                                                    |
...
| ID    | Item                                                        | Status                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1.1  | Order create + facial-measurement mirror transaction        | **RECONSIDERED — not a fix.** `routes/storefront/orders.ts:124-131` and `:191-197` _intentionally_ catch and continue on each write's failure: "the patient's primary expectation is that Penn receives the order; losing the audit row is bad but recoverable." Wrapping in a transaction would couple the failure modes and defeat the design. Mirror is recoverable from `orders.payload` (line 117). Recommend dropping from backlog. |
...
| 6   | N3          | Per-document audit row in HIPAA sweep — `logAuditBestEffort` per flagged row.                    | S      |
| 7   | P1.3        | Apply CSRF middleware to storefront state-changing endpoints.                                    | M      |
| 8   | P2.1 slice  | Extract one tab from `patient-detail.tsx` to set the precedent and stop the growth.              | S      |

P1.1 ("wrap order create + mirror in a transaction") was on this list
in an earlier draft. On closer reading of the call site, it's been
**reconsidered as a non-fix** — both writes are deliberately
best-effort per the inline comments at `orders.ts:124-131` and
`:191-197`, and the mirror is recoverable from `orders.payload`.
Wrapping in a transaction would defeat the design intent. See the
P1.1 row in the status table above.
