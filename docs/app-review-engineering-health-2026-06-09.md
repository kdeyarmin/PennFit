...
| W2  | ✅ **Implemented (this PR).** Added `worker/lib/integration-health.ts`: tracks consecutive failures per cron in `resupply.integration_run_health` (migration 0261); escalates SFTP list failures from `warn` → `error` and emails `RESUPPLY_ADMIN_EMAILS` after ≥3 consecutive failures (repeating every 5). Therapy nightly sync alerts when ≥80% of patients fail in one run; Office Ally poller alerts on the SFTP list itself failing. | done   | `worker/lib/integration-health.ts`, `therapy-integrations-nightly-sync.ts`, `office-ally-inbound-poll.ts`, migration 0261 |
