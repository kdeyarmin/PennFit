...
| HSAT capture (C-R3)                     | ✅ SHIPPED\* | `routes/patients/sleep-studies.ts` (`study_type='hsat'`, `source='home_test_vendor'`) + patient self-report `routes/shop/me-sleep-study.ts`. \*In-funnel *vendor ordering* still partner-gated.       |
...
W3 DLQ admin panel · F2 product JSON-LD · F3 Web-Vitals RUM · F8 results
