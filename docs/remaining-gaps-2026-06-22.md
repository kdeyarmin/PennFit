...
- **JS-side aggregation caps.** Several `.limit(20000/50000)`-then-aggregate-in-JS
  ...
  `mask-catalog` formulary reads, `business-targets` metrics*daily, and
  related). Silent truncation past PostgREST `max_rows` (~1000). Prefer SQL
  RPCs or newest-first paging + `windowTruncated`. **Shipped / mitigated:**
  `ltv-cac` + `resupply-kpis` RPCs (#1209); `billing-benchmarks` /
  ...
  `rt-outcomes` (this branch). \_Effort: S–M for residual sites.*
