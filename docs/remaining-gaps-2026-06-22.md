...
- **JS-side aggregation caps.** Several `.limit(20000/50000)`-then-aggregate-in-JS
  ...
  `mask-catalog` formulary reads, and related). Silent truncation past
  PostgREST `max_rows` (~1000). Prefer SQL RPCs or newest-first paging +
  `windowTruncated`. **Shipped / mitigated:** `ltv-cac` + `resupply-kpis`
  RPCs (#1209); `billing-benchmarks` / `voice-metrics` / `mask-fit-worklist`
  keyset or range paging; `staffing-live` (#1350); collections-forecast +
  forward-order-book (#1351); `payer-profitability` (#1352); aging-report +
  dso-by-payer (#1353); `rt-outcomes` (#1354); `business-targets`
  metrics*daily (this branch). \_Effort: S–M for residual sites.*
