...
| Med  | `eligibility-verifier` / `era-reconciler` default `orgId` to seed if caller omits (live callers pass it; footgun for new sites)                                | **Shipped** — both throw without orgId; statement / claim-status / discovery / ERA payer / denial scorer / subscription notice no longer invent seed |
| Low  | Denied-primary COB explicitly unfinished                                                                                                                       | `secondary-cob.ts` comments                                                                                                                          |
>>>>>>> d12a2cd98 (fix: require orgId on billing helpers (no seed invent))
