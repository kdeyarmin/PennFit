...
| **Secondary / COB auto-submit**              | **PARTIAL**               | auto-**draft** is automated — auto-workflow Pass 4 (`lib/billing/auto-workflow-engine.ts:366`, flag `billing.auto_secondary_claims`) drafts a secondary for each paid primary with secondary coverage + a patient balance; the draft is **not excluded from auto-submit** (`lib/billing/auto-submit-engine.ts:278` selects any `status='draft'`), so a preflight-clean secondary submits with the `billing.auto_submit_claims` batch. Manual path: `generateSecondaryClaimDraft` (`routes/admin/secondary-claims.ts:104`) | Effectively end-to-end with both flags on; the residual gap is a secondary needing primary-EOB / COB completeness to clear preflight. |
| **Prior-auth automation**                    | **PARTIAL**               | Da Vinci PAS submit is **manual-only** (`routes/admin/davinci-pas-submit.ts`); no worker / auto-engine invokes `submitPasBundle` (grep-confirmed none); no bulk-PA engine; per-payer token is process-env `DAVINCI_PAS_TOKEN_<PAYER_SLUG>` (`davinci-pas-submit.ts:289`), not in `clearinghouse_credentials`                                                                                                                                                                                                              | PA-required items wait on a manual click; front-loading PAs prevents the denial.                                                      |
| **Appeals lifecycle tracking**               | **PARTIAL**               | appeal **letters** tracked in `claim_appeal_letters` (migration 0137) with `delivery_method` / `delivered_at`; the **fax** path auto-transitions denied → appealed on Telnyx success (`routes/admin/claim-appeals.ts:357`), but **mail / email / portal** deliveries have no system "sent" event so they neither transition nor age; **no `responded_at` / `outcome` / aging columns** (verified absent in 0137)                                                                                                          | Can't measure appeal **win-rate** (no outcome) or age the appeal clock; a mail/email appeal silently stays "denied."                  |
