...
3. **Setup Guide** — what must be configured before going live, and an
   explanation of **every Control Center toggle** (default state + what it
   does when on/off).
4. **Part 1 — Feature Summary by Role** — a one-line description of every
   feature, grouped by role (Administrator, Biller, CSR, Respiratory
   Therapist).
5. **Part 2 — Comprehensive Feature Reference by Role** — the full detail on
   each feature.
6. **Part 3 — Job Aides by Role** — curated step-by-step walkthroughs for the
   highest-value tasks.
7. **Appendix** — role/permission matrix + glossary.
