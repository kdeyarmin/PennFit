...
| Resolving *which* org (the directory itself) | `resolveSeedOrgId()` / a future `resolveOrgBy…` — not the scoped client |
...
- **Loosely-typed builder returns.** `db.from(...)` methods currently
  ...
  invest in fully-typed facade returns *before* the broad fan-out — every
...
1. **`org_id` → `NOT NULL`.** Valid only once *every* insert path supplies
