...
| ~~O2~~ | ~~Purchasing / PO + supplier management~~       | ⛔ **out of scope by architecture** (correction)                               | Attempting this tripped **Rule 14** in `scripts/check-resupply-architecture.sh`: PennFit's DB may **not** own purchasing/inventory tables (`purchase_orders`, `warehouses`, receiving, …) — **Pacware is the inventory system of record**, and a half-WMS here would split the source of truth. Belongs in Pacware integration code or an ADR to move the boundary — not a PennFit gap. **Dropped.** | —                 | `scripts/check-resupply-architecture.sh` (Rule 14), `docs/resupply/ARCHITECTURE.md` |
