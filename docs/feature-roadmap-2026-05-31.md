...
| 3   | **LTV & CAC cohort economics** (lifetime value per patient; acquisition cost by source)                  | L    | Cohorts from signup month + `referrals_attribute` for channel; **F1** for margin-based LTV. Aggregates only.                                                            |
