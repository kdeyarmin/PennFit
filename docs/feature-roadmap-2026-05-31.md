...
1. **Migrations** are hand-written SQL in `lib/resupply-db/drizzle/`.
   The next free prefix is **`0206`** (this branch uses `0188`–`0205`;
...
| 22a | **✅ Mask-fit confirmation loop — capture** (post-fit feedback: seal good / leaking / uncomfortable)                | M    | ◑ **Slice 1 (capture loop):** migration `0201` `mask_fit_outcomes` (bound to a shop_order, RT triage state machine new→reviewed→actioned). Mirrors the NPS micro-survey exactly — `lib/mask-fit-token.ts` HMAC-signed link (good/leaking/uncomfortable × order, 30-day TTL, `RESUPPLY_LINK_HMAC_KEY`, no new secret) → public `POST /shop/orders/mask-fit` (IP-rate-limited, token-verified) → storefront `/mask-fit` survey page. ✅ **Slice 2 (RT worklist):** pure tested `rankMaskFitWorklist` (worst-fit first); `GET /admin/clinical/mask-fit/worklist` (clinical.read, patient ids resolved from the order) + `POST …/:id/triage` (clinical.intervention.write, new→reviewed→actioned); SPA "Mask-fit feedback" page (Clinical nav). Token + both routes unit-tested. (The feedback→rec-engine learning loop is §6/22b.)                                                                                                                                                           |
| 23  | **✅ Clinical outreach** (supportive check-ins to non-adherent patients) _(owner-authorized outward)_               | M    | ✅ Proactive, templated outreach off the #21 open interventions. Migration `0204` `clinical_outreach_log` (audit + frequency-cap source). Pure tested `lib/clinical/clinical-outreach.ts`: `outreachChannelAllowed` mirrors the smart-trigger clinical-nudge policy EXACTLY (DND always blocks; no prefs row → allowed; else channel marketing opt-in), `pickOutreachChannel`, per-`assessment_category` `buildOutreachMessage`, `selectOutreachTargets` (one/patient/run, frequency-capped, capped), `sendOneOutreach`/`runClinicalOutreachBatch` (consent/DND-gated, one From, "Reply STOP", logged; senders injectable for tests). `GET /admin/clinical/outreach/eligible` (clinical.read) + `POST …/run` (clinical.intervention.write); `clinical.outreach-batch` pg-boss job with an **opt-in cron** (`CLINICAL_OUTREACH_CRON`, off by default). SPA "Clinical outreach" page (Clinical nav). Built after explicit owner authorization. Counts/category/channel/status only in logs. |
...
| 29  | **✅ CMN / DIF (structured forms)** (Certificates of Medical Necessity / DME Information Forms)          | L    | ◑ **Backend shipped.** Note: `dwo_documents` (0134) already tracks CMN/DWO/SWO renewal + signed PDF + expiry; per owner decision, built the dedicated **structured-form** complement it lacks. Migration `0202` `cmn_documents` (per-patient, optional claim + soft `dwo_document_id` link, `answers` jsonb, status draft→completed→on*file→voided). Pure tested `lib/billing/cmn-forms.ts` — a curated CMS form catalog (CMS-484 oxygen, 846, 848, DIF 10125/10126) with per-form `requiredKeys` + `validateCmnAnswers` + `formTypeForHcpcs`/`hcpcsRequiresCmn`. CRUD: `GET/POST /admin/patients/:id/cmn-documents`, `PATCH /admin/cmn-documents/:id` (**refuses `completed` until the form validates**, returns the gaps), `GET /admin/billing/cmn-catalog` + `cmn-worklist` (drafts + readiness). Forms + routes unit-tested (13). **Slice 2 (SPA):** patient-page CMN card (create + catalog-driven answer editor + Complete with inline gap feedback) + `/admin/billing/cmn` draft worklist (Billing nav). \_CMS retired most CMN/DIF 1/1/2023 — built at owner request despite the dwo_documents overlap.*                                                                                                                                                                                                                                                              |
...
| #   | Item                                                                                | Size | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --- | ----------------------------------------------------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4   | **✅ Revenue forecast / cash-flow projection**                                      | L    | ◑ **Slice 1 (AR collections):** pure tested `projectClaimCollections` — expected cash from outstanding (submitted/accepted) claims, bucketed into ≤30/31–60/61–90/>90-day horizons. Honest, query-tunable assumptions (`expectedDaysToPay`, `defaultAllowedRatio` for billed→allowed when unadjudicated, `collectionProbability`) echoed in the response. `GET /admin/billing/collections-forecast` (reports.read) + owner "Collections forecast" page (Billing nav) with inline what-if assumption inputs. **Slice 2 (forward order book):** pure tested `projectForwardOrderBook` — expected NEW resupply revenue from prescriptions becoming eligible (real last-fill + `cadence_days`) bucketed ≤30/31–60/61–90, × tunable expected-order-value × confirm-rate (labeled estimates). `GET /admin/billing/forward-order-book` + a second card on the forecast page.                                                                                                               |
| 22b | **✅ Mask-fit feedback → rec-engine tuning loop** _(mechanism; neutral until data)_ | M–L  | ✅ Mechanism shipped (built at owner request despite no accumulated data — a no-op until outcomes accrue). Pure tested `lib/storefront/mask-fit-tuning.ts`: `computeFitAdjustments` turns per-mask good/leaking/uncomfortable counts into a bounded (±0.15) per-mask RANKING multiplier, **neutral below `minSamples` (10)**; `tallyOutcomesByMask` folds rows. `recommend()` gains an optional `fitAdjustments` arg applied to the **sort score only** (like the manufacturer boost — excluded from patient-facing confidence; default neutral, so live behavior is unchanged + the 20 engine tests stay green). Data path: migration `0203` adds `mask_id` to `mask_fit_outcomes`, carried in the signed survey token + persisted on capture; `GET /admin/clinical/mask-fit/rec-signal` (clinical.read) surfaces per-mask counts + the multiplier each would feed. Wiring `fitAdjustments` into the live recommend route is a deliberate switch to flip once the signal is dense. |
| 25  | **✅ Short-video education library** (mask fitting, ramp, cleaning)                 | M    | ✅ Migration `0205` `education_videos` (title, topic, https `video_url`, thumbnail, duration, sort, active). **Vendor-agnostic + fail-soft** per ground rule 9 — `video_url` is a BYO link (YouTube / Vimeo / CDN), no SDK/key/boot-risk; surfaces degrade to an empty state on an empty catalog. Pure tested `lib/storefront/education-videos.ts` (topic catalog + `activeVideosInOrder` / `groupActiveVideosByTopic`). Public `GET /shop/education-videos` (grouped, fail-soft); admin CRUD `GET/POST/PATCH /admin/education-videos` (reports.read / admin.tools.manage, https-only URL validation). Storefront `/learn/videos` page (grouped cards, linked from the `/learn` hero) + admin "Video library" management page (Clinical nav). Pure + route tests (12). _Add real clips by pasting hosted URLs — no vendor onboarding needed._                                                                                                                                       |
...
| 23  | RT      | ✅ Clinical outreach               | 3      | M       | F3              |
...
| 29  | Biller  | ✅ CMN / DIF (structured forms)    | 5      | L       | core            |
