...
1. **Migrations** are hand-written SQL in `lib/resupply-db/drizzle/`.
   The next free prefix is **`0198`** (this branch uses `0188`–`0197`;
...
| 11  | **◑ Click-to-dial + call-back queue** (dial from the patient panel; auto-log disposition)                    | L    | ◑ Core shipped (slice 1): migration `0197` (`call_dispositions` + `admin_users.phone_e164`); `POST /admin/patients/:id/click-to-dial` places an **agent-first Twilio bridge** (rings the CSR's own phone first via `placeCall`, then the signed `/voice/click-to-dial-twiml` webhook `<Dial>`s the patient — number never reaches the browser) behind a pure tested 9am–7pm-ET call-window guard (overridable, recorded); `POST /admin/call-dispositions/:id` logs the outcome + note (note never logged — PHI). New `buildDialTwiml` builder. SPA `ClickToDialCard` on the patient page (Call → guardrail-aware → disposition form). **Deferred to slice 2:** the call-back _queue_ page + the agent set-your-number UI. Gated `conversations.manage`. |
...
| 11  | CSR     | ◑ Click-to-dial + call-back queue  | 4      | L       | core (F4)       |
