...
| 36  | **◑ Timely-filing countdown** on every open claim ("X days left to file/appeal")                       | S    | ◑ Core shipped: pure `timelyFilingStatus` in `@workspace/resupply-domain` (deadline = `date_of_service` + `payer_profiles.timely_filing_days`; ok / due_soon / overdue / unknown). Remaining: surface it on the `admin-billing-*` claim rows.                                                                                       |
...
| 8   | **✅ Goal / target tracking** (set a monthly net-collections / new-patient target, watch pace-to-goal) | S    | ✅ Shipped: `business_targets` table (migration 0190) + `/admin/business-targets` API (list / upsert, gated `targets.manage`). Pace-to-goal automation (joining F2 `metrics_daily` actuals) lands with the F2 wiring.                                                                                                               |
