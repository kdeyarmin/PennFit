...
metrics**). Remaining genuinely-open code item: **C2 (inbound-fax OCR)** — needs an OCR
vendor/runtime decision before it can be built. The only other gaps are voice-call
*timing* metrics (blocked on the no-op call-event audit stub) and the owner runtime
activations above.
