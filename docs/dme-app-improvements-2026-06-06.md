...
> research and the 2026-05-31 feature review. This doc re-scopes _all three
> internal personas_ against what is actually in the tree today and tags every
...
inbound _and_ outbound, Claude post-call summary), a **~99-page admin console /
...
therefore the _next layer_: **activating what's already built**, **a few genuinely
...
| 🟡 **Built-but-dormant** | Code exists; blocked on an _activation decision_, _data volume_, or a _partner contract_ — not on engineering. |
...
| O2  | **Insurance estimate that learns from real outcomes.** ✅ (this PR) — a weekly worker classifies adjudicated-claim payer names → storefront slugs and computes **P50/P90 patient OOP** into a small `payer_estimate_stats` table (RPC `payer_oop_samples`, mig `0230`); the public estimate route reads it and shows a data-derived "what {payer} patients actually paid" range alongside the static one, falling back to static below 20 samples.                                                                                                                                 | ✅ (this PR)     | Higher checkout conversion + fewer "surprise bill" complaints; turns a guess into a data-backed range.                                            | done                 | `lib/insurance-estimates/{learn,refresh-stats}.ts`, `worker/jobs/payer-estimate-stats-refresh.ts`, `routes/shop/insurance-estimate.ts`, `pages/insurance-estimate.tsx` |
| O3  | **Predictive denial scoring at claim preflight.** Surface "payer X denied 38% of recent E0601 claims" _as a non-blocking preflight warning_ before submission, from the claim's own (payer × HCPCS) history.                                                                                                                                                                                                                                                                                                                                                                       | ✅ **(this PR)** | Directly cuts the denial rate (~2–5% of DME revenue in write-offs/rework) and DSO.                                                                | M (done)             | `lib/billing/denial-risk.ts`, `lib/billing/claim-preflight.ts`, RPC migration `0228`                                                                                   |
| O4  | **Resupply KPI benchmark tiles.** Connection / confirmation / fulfillment rates + orders-per-patient/yr were already shipped; **this PR adds items-per-order (fulfillment line items / order) and AOV (paid storefront orders)** to the same `resupply-kpis` endpoint + analytics tiles. **Retention** ✅ (this PR) — `GET /admin/analytics/patient-retention` measures repeat-supply rate + active/lapsed + repeat-by-cohort on fulfilled episodes (no new churn marker needed; the fulfilled-episode history _is_ the activity signal), surfaced as a "Patient retention" panel. | ✅ (this PR)     | "Measure it to manage it"; benchmark against published resupply norms.                                                                            | done                 | `lib/analytics/aggregate.ts`, `routes/admin/analytics.ts`, `pages/admin/admin-analytics.tsx`                                                                           |
...
paperwork**. Note: the strongest RT items are _activation/data_ problems, not
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1  | **Patient dedup / merge.** ✅ (this PR) — detection (`GET /patients/duplicates`, RPC `0229`) + a `/admin/patients/duplicates` review page where a CSR picks the survivor and merges duplicates in. The merge (`POST /patients/merge` → RPC `merge_patient_records`, mig `0225`) atomically repoints every FK to `patients(id)`, refuses on any unique conflict (all-or-nothing), and closes (not deletes) the duplicate. The dynamic cross-table SQL is validated against real in-process Postgres (pglite) — atomic rollback on a unique-constraint conflict demonstrated.                                                                                                                                                       | ✅ (this PR)              | Kills the most common DME data-hygiene headache; one clean patient record instead of three.                     | done                      | `routes/patients/{duplicates,merge}.ts`, RPCs `0229`/`0225`, `pages/admin/admin-patients-duplicates.tsx`                                                                                                                                      |
| C2  | **Inbound fax → structured fields (OCR).** ✅ (this PR) — `POST /admin/inbound-faxes/:id/ocr` runs the fax page through the **existing BAA-covered Claude vision/PDF path** (no new OCR vendor) and extracts patient/physician/items/doc-type into JSON persisted on the row (`ocr_*` cols, mig `0226`). The triage modal gains an "AI field extraction" panel: detected fields as copy-able hints, auto-applied document category, "use as note". Fail-soft: no model key → `offline`, CSR keys by hand as before. PHI-safe (page bytes + extracted fields never logged).                                                                                                                                                        | ✅ (this PR)              | Removes the heaviest manual data-entry step in DME intake.                                                      | done                      | `lib/inbound-fax/ocr.ts`, `routes/admin/inbound-faxes.ts`, `pages/admin/admin-inbound-faxes.tsx`, `lib/resupply-ai/anthropic-client.ts`                                                                                                       |
| C3  | **Real-time staffing / queue dashboard.** ✅ (this PR) — `GET /admin/staffing/live` + a **Live staffing** console page show per-agent open-conversation load right now, availability, who's on shift, and the unassigned backlog (auto-refreshing). **Voice handoff queue** ✅ (this PR) — `/admin/ops-status` returns `voiceHandoffs: { open, urgent }` (live escalated queue flagged by the post-call summarizer; urgent = distressed). **Voice-call timing** ✅ (this PR) — a new `resupply.voice_calls` ledger (mig `0227`) is populated best-effort from `/voice/status-callback`; `GET /admin/voice/metrics` reports volume, answer rate, handle time, and ring time, rendered as a "Voice calls — last 30 days" ops panel. | ✅ (this PR)              | Lets a lead rebalance load mid-shift, triage distressed callers first, and see call throughput.                 | done                      | `lib/staffing/build-live-staffing.ts`, `routes/admin/staffing-live.ts`, `routes/admin/ops-status.ts`, `lib/voice/voice-call-record.ts`, `lib/analytics/voice-metrics.ts`, `routes/admin/voice-metrics.ts`, `pages/admin/admin-operations.tsx` |
...
compliance is handled out of band, so these are mostly _operational_ with a thin
code surface.)
...
1. **O1 — activate storefront auto-reminder enrollment** (owner): an XS flag flip
   ...
   _Owner runtime/consent decision — not a code change._
2. **C4 — auto-action obvious queues** (CSR): turn on the built cart-abandonment
   cron + a failed-order digest; immediate CSR-labor relief. _Mostly runtime flags._
...
4. **O3 — predictive denial scoring at preflight** (owner): ✅ **implemented in this
   ...
   denial history (`lib/billing/denial-risk.ts` + RPC migration `0228`).
...
shipped. **Delivered in this PR:** O3, R3, C1 (detection **and** the atomic merge,
real-Postgres-validated via pglite), O4 (KPI tiles **incl. patient retention**), O2
(learning insurance estimate), C3 (real-time staffing **incl. voice-handoff queue +
voice-call timing metrics** — a new `voice_calls` ledger replaces the no-op audit
stub for telemetry), and **C2 (inbound-fax OCR via the existing Claude vision path —
no new vendor)**. Every genuinely-open item from the persona tables is now done. The
only remaining work is the owner runtime/consent activations (O1 / C4 — flip a
flag/env, not code).
