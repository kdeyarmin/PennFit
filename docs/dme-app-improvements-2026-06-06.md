...
| - | ----------- | -------- | -------------------- | ------ | --------- |
| C1 | **Patient dedup / merge.** ✅ (this PR) — detection (`GET /patients/duplicates`, RPC `0223`) + a `/admin/patients/duplicates` review page where a CSR picks the survivor and merges duplicates in. The merge (`POST /patients/merge` → RPC `merge_patient_records`, mig `0225`) atomically repoints every FK to `patients(id)`, refuses on any unique conflict (all-or-nothing), and closes (not deletes) the duplicate. ⚠️ The dynamic cross-table SQL is unit/route-tested with mocks but isn't exercised against a real DB in CI — validate on a staging copy before first production use. | ✅ (this PR) | Kills the most common DME data-hygiene headache; one clean patient record instead of three. | done | `routes/patients/{duplicates,merge}.ts`, RPCs `0223`/`0225`, `pages/admin/admin-patients-duplicates.tsx` |
