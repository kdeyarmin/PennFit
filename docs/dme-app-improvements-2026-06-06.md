...
| - | ----------- | -------- | -------------------- | ------ | --------- |
| C1 | **Patient dedup / merge.** Only `pacware_id` uniqueness is enforced; fax/referral intake produces variant spellings. **Detection + console page** ✅ done (this PR) — `GET /patients/duplicates` (RPC, same DOB+last name / phone / email) feeds a `/admin/patients/duplicates` review page. The destructive **merge** (repoint every patient_id FK) remains 🔴 — a deliberate follow-up that needs a real-DB test harness. | ✅/🔴 | Kills the most common DME data-hygiene headache; one clean patient record instead of three. | detection+UI done; merge M–L | `routes/patients/duplicates.ts`, RPC `0223`, `pages/admin/admin-patients-duplicates.tsx` |
