...
| O4 | **Resupply KPI benchmark tiles.** Connection / confirmation / fulfillment rates + orders-per-patient/yr were already shipped; **this PR adds items-per-order (fulfillment line items / order) and AOV (paid storefront orders)** to the same `resupply-kpis` endpoint + analytics tiles. **Retention** stays 🔴 — not computable without a churn/last-active marker (deferred). | ✅ (this PR) / 🔴 retention | "Measure it to manage it"; benchmark against published resupply norms. | items+AOV done | `lib/analytics/aggregate.ts`, `routes/admin/analytics.ts`, `pages/admin/admin-analytics.tsx` |
...
| C3 | **Real-time staffing / queue dashboard.** ✅ (this PR) — `GET /admin/staffing/live` + a **Live staffing** console page show per-agent open-conversation load right now, availability, who's on shift, and the unassigned backlog (auto-refreshing). Voice-queue wait/handle time is a 🔴 follow-up (needs the voice-call event rollup). | ✅ (this PR) / 🔴 voice queue | Lets a lead rebalance load mid-shift instead of reading lagging close-rate rollups. | done | `lib/staffing/build-live-staffing.ts`, `routes/admin/staffing-live.ts`, `pages/admin/admin-live-staffing.tsx` |
