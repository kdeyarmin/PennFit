...
| C3 | **Real-time staffing / queue dashboard.** ✅ (this PR) — `GET /admin/staffing/live` + a **Live staffing** console page show per-agent open-conversation load right now, availability, who's on shift, and the unassigned backlog (auto-refreshing). **Voice handoff queue** ✅ (this PR) — `/admin/ops-status` now returns `voiceHandoffs: { open, urgent }` (live escalated queue flagged by the post-call summarizer; urgent = distressed) rendered as a Voice-handoffs panel on the operations page. Voice-call wait/handle *timing* stays out of scope (the call-event audit rows are a no-op stub). | ✅ (this PR) | Lets a lead rebalance load mid-shift, and triage distressed callers first. | done | `lib/staffing/build-live-staffing.ts`, `routes/admin/staffing-live.ts`, `routes/admin/ops-status.ts`, `pages/admin/admin-operations.tsx` |
...
metrics**), and **C2 (inbound-fax OCR via the existing Claude vision path — no new
vendor)**. Every genuinely-open *code* item from the persona tables is now done. The
only remaining gaps are non-code: voice-call *timing* metrics (blocked on the no-op
call-event audit stub) and the owner runtime/consent activations above.
