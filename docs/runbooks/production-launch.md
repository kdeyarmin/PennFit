...
| `RESUPPLY_VOICE_PUBLIC_BASE_URL`     | `https://pennpaps.com` — telephony webhook target: Twilio (voice agent + SMS) and Telnyx (fax). The voice agent is **optional**; to turn it on, follow [`voice-agent-go-live.md`](./voice-agent-go-live.md) to provision the number and wire its webhooks. |
