...
*actually not built* and would move the target metrics is short:
...
| "No secondary-claims feature" | **Shipped** (manual trigger — see 2.1 for the *automation* gap). | `routes/admin/secondary-claims.ts` |
...
by writing new features. The only genuinely worthwhile *new* build is
auto-drafting secondary/COB claims. Everything else is activation, business
development, or scale hygiene.

> **Caveat on flag state:** this analysis reads the *seeded* defaults from
