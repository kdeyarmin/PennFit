...
senders; two **order-confirmation** senders remain to be metered (see the
G12 row). G9 (per-tenant brand **colors**) has been **decided as a
...
layer (G10, G16, and G12's infrastructure + metric set) is done; the
remaining code items are two G12 order-confirmation wire-ups, the G1
signed reminder-link tenant resolution, optional/later items (G11,
G13–G15), and the cron-fan-out session's SUITE-GATED tail.
...
(G10, G16, plus G12's table + metric set) are merged. G9 is a **product
non-goal** (consistent theme by design). The remaining code items are
narrow: (a) the **G1** signed reminder-link tenant resolution — the
`/api/reminders/manage` click-through still resolves the seed org and reads
the global `reminder_subscriptions` table (`routes/storefront/reminders.ts`),
coupled to the reminders sender workstream; (b) two **G12**
order-confirmation senders to meter; and (c) the **G2** SUITE-GATED cron
tail owned by the cron-fan-out workstream. None block a second tenant from
being served, transacting, or billing; (a) and (b) affect tenant-correct
reminder routing and complete usage counts.
