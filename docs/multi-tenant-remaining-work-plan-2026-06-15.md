...
| **G1**      | Storefront/customer served by host, not seed | **Done.** `resolveOrgIdByHost()` + `requireSignedIn`/`attachSignedIn` resolve the tenant by host; `customerIdResolver`/`ensureShopCustomerRow` thread the host org; cross-tenant **serving** test is `middlewares/requireSignedIn.tenant-host.test.ts`. **Signed-link tenant resolution complete (2026-06-17):** every public HMAC-token route resolves the tenant FROM the referenced record via `resolveOrgIdForSignedRecord(table, id)` (the sanctioned cross-tenant `.raw()` read), not host/seed. The three that still pinned to seed/host — `video-visit-session` (`video_visits`), `rx-request-document` (`prescription_request_packets`), `nps-response` (`shop_orders`) — are now record-based; the reminders `/reminders/manage*` routes already operate token-keyed `.raw()` on the global `reminder_subscriptions` table (seed id is only a raw handle, never a filter) and brand the unsubscribe from the row's own `org_id`, so they were already tenant-correct. |
...
by design). **G1 signed-link tenant resolution is now closed (2026-06-17):**
the three public token routes that still pinned to seed/host
(`video-visit-session`, `rx-request-document`, `nps-response`) resolve the
tenant from their referenced record via `resolveOrgIdForSignedRecord`, joining
the email-click / packet / fitter / mask-fit routes already on that pattern;
the `/reminders/manage*` routes were already tenant-correct (token-keyed
`.raw()` on the global table, unsubscribe branded from the row's `org_id`).
The remaining code item is (b) the **G2** SUITE-GATED cron tail owned by the
